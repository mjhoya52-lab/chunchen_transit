...
st.markdown(
    ...
    <style>
      ...
      .route-card {
        ...
        border-radius: 18px; padding: 22px; min-height: 360px; margin: 14px 0; box-shadow: 0 8px 26px rgba(44,70,50,.06);
      }
      ...
      .time { font-size:25px; font-weight:900; letter-spacing:-1px; line-height:1.15; }
      .type, .arrival small { color:#89908b; font-size:10px; }
      .arrival { text-align:right; white-space:nowrap; }.arrival strong { display:block; color:var(--green-dark); font-size:16px; margin-top:3px; }
      ...
      .walks { display:flex; gap:7px; flex-wrap:wrap; margin-bottom:13px; }.walks span { padding:6px 9px; border-radius:9px; background:#f1f4ef; color:#69716b; font-size:10px; }.walks b { color:var(--ink); }
      .journey { padding:13px; background:#f8faf7; border-radius:13px; display:grid; gap:10px; }
      .segment { display:grid; grid-template-columns:55px 1fr; gap:10px; }.bus-number { display:grid; place-items:center; min-height:29px; padding:4px; border-radius:8px; background:var(--green); color:#fff; font-size:10px; font-weight:900; text-align:center; }.segment strong { display:block; font-size:12px; }.segment small { color:#858d87; font-size:10px; }.realtime { color:#d9342b; font-weight:900; }
      .stats { display:grid; grid-template-columns:repeat(3,1fr); margin-top:15px; padding-top:13px; border-top:1px solid var(--line); }.stats div { padding:0 7px; border-right:1px solid var(--line); }.stats div:last-child { border:0; }.stats small { display:block; color:#919892; font-size:9px; }.stats b { font-size:12px; }
...
def render_route(route: dict) -> None:
    ...
    for segment in route["segments"]:
        ...
        segments_html += f"""
          <div class="segment"><span class="bus-number">{escape(', '.join(segment['route_numbers']))}</span>
          <span><strong>{escape(segment['boarding_stop'])} → {escape(segment['alighting_stop'])}</strong>
          <small>정류장 {segment['stop_count']}개 · 버스 탑승 {escape(segment['in_vehicle_text'])}{wait}</small></span></div>"""
    st.markdown(
        f"""
