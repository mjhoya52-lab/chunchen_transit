...
def apply_realtime_predictions(
    ...
    """
    상위 경로의 각 버스 구간에 춘천버스GO 실시간 도착정보를 붙인다.
    ...
    # 동일 정류장의 노선별 요청을 하나로 합치고, 이후에는 캐시를 사용한다.
    # 후보 수와 정렬 기준은 그대로 유지한다.
    ...
    if lookup_node_ids:
        with ThreadPoolExecutor(
            max_workers=min(8, len(lookup_node_ids))
        ) as executor:
            futures = [
                executor.submit(
    ...
    for result_index, result in enumerate(
        ...
        for segment in result["segments"]:
            ...
            except RealtimeApiError as error:
                segment[
                    "realtime_status"
                ] = "error"
                segment[
